...
> **Limited applicability:** TRT VAE provides only ~1.2x speedup over PyTorch VAE at typical resolutions (benchmarked at 1216×832 on RTX 4060 Ti: 0.59s vs 0.73s per decode). In standard txt2img workflows where UNet sampling dominates, this has negligible impact on total generation time. The speedup scales with resolution, so TRT VAE is most useful for high-resolution or VAE-heavy workflows (batched decode, img2img loops) where decode time is a meaningful fraction of total time. Evaluate whether the build time is justified for your specific workflow before using.
